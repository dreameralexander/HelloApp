    ...
    class Program 
    {
        ...
        static void Main(string[] arg)
        {
            ...
            */
            ...
            //byte a = 4;
            //int b = a + 70;     //Результат операции равен 74

            //byte a = 4;
            //byte b = a + 70;        //Ошибка

            //Console.Write($"\nbyte a = 4;\nint b = a + 70;     //Результат операции равен 74");
            //Console.Write($"\nbyte a = 4;\nbyte b = a + 70;        //Ошибка, при операциях мы должны учитывать диапозон значений который может хранить тот или иной тип.\n" +
            //    $"Число 74 укладывается в диапозон значений byte, но мы получаем ошибку. " +
            //    $"Операция + и - возращает значение типа int, если в операции участвуют целочисленые типы данных меньше или равно int.\n" +
            //    $"Поэтому результат операции a + 70 вернет объект который имеет длину 4 байта в памяти (int)," +
            //    $"который мы пытаемся присвоить переменной byte (1 байт).\n" +
            //    $"Это приводит к ошибке.");
            //Console.Write($"\nНеобходимо применить операцию преобразования типов" +
            //    $"\nbyte a = 4;\nbyte b = (byte)(a + 70);     //Результат операции равен 74");
            ...
            c = a > b;
            Console.Write($"\nОперации сравнения > (больше чем)\nbyte a = 10, b = 4;\nbool c = a({a}) > b({b}); //c = {c}");
            c = a < b;
            Console.Write($"\nОперации сравнения > (меньше чем)\nbyte a = 10, b = 4;\nbool c = a({a}) < b({b}); //c = {c}");
            c = a >= b;
            Console.Write($"\nОперации сравнения >= (больше чем или равно)\nbyte a = 10, b = 4;\nbool c = a({a}) >= b({b}); //c = {c}");
            c = a <= b;
            Console.Write($"\nОперации сравнения >= (меньше чем или равно)\nbyte a = 10, b = 4;\nbool c = a({a}) <= b({b}); //c = {c}");

            //Логические операции
            bool x = false, y = true, z;
            z = x | y;
            Console.Write($"\nЛогические операции | (логическое или)\nz({z}) = x({x}) | y({y});");
            z = x & y;
